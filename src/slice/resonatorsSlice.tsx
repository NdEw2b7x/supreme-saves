...
import { everyResonatorData, isRover, ResonatorCode } from 'libs/Resonators'

const name = 'resonatorsSlice'

let roverElement: ResonatorElement = 'dark'
...
let initialState: { 공명자: MyResonator[]; element: ResonatorElement } = {
  공명자: [defaultRover],
  element: 'dark',
}
...
const reducers = {
  ...
  changeElement: (
    state: InitialState,
    { payload }: { payload: ResonatorElement }
  ) => {
  ...
  changeRank: (
    ...
    {
      payload: { code, rank },
    }: { payload: { code: ResonatorCode; rank: Rank } }
  ) => {
  ...
  changeLevel: (
    ...
    {
      payload: { code, level },
    }: { payload: { code: ResonatorCode; level: number } }
  ) => {
    const current = Object.fromEntries(state['공명자'].map(i => [i['코드'], i]))
    const getResonator = current[code]
  ...
  changeChain: (
    ...
    {
      payload: { code, chain },
    }: { payload: { code: ResonatorCode; chain: EveryChain } }
  ) => {
    const current = Object.fromEntries(state['공명자'].map(i => [i['코드'], i]))
