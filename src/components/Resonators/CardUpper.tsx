...
import { codeConverter, everyResonatorData } from '../../libs/Resonators'
...
export const getMyHarmony: (
  x: (MyEcho | undefined)[]
) => Partial<Record<Harmony, number>> = (x: (MyEcho | undefined)[]) => {
  ...
  return Object.fromEntries(
    Object.entries(countHarmony).filter(([, c]) => c >= 2)
  )
}
  ...
  return (
    <div className={styles.top}>
      <div className={styles.intro}>
        ...
        <div className={styles.imgBox}>
          {weaponThumbnailControl(myWeapon?.코드)}
        </div>
      </div>
      ...
      <div className={styles.stats}>
        <div>
          <span>HP</span>
          <span>
            {(result.baseHp * (1 + result.hp) + result.flatHp).toFixed(3)}
          </span>
        </div>
        <div>
          <span>공격력</span>
          <span>
            {(result.baseAtk * (1 + result.atk) + result.flatAtk).toFixed(3)}
          </span>
        </div>
        <div>
          <span>방어력</span>
          <span>
            {(result.baseDef * (1 + result.def) + result.flatDef).toFixed(3)}
          </span>
        </div>
        {(
          [
          ...
          .map(i => (
            <div key={i}>
              <span style={{ color: `var(--element-${i})` }}>
                {mapStatsName[i]}
              </span>
